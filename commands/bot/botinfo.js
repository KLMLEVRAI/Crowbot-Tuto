...
module.exports = {
	...
	run: async (client, message, args, prefix, color) => {
		...
		if (client.config.owner.includes(message.author.id)) {
            ...
            embed.setURL('https://discord.gg/omaji')
            embed.setDescription('**Une description complète et détaillée du Bot** <@658657618520440832>')
            embed.setColor(color)
            .setThumbnail(message.author.avatarURL({ dynamic:true }));
            embed.setTimestamp()
            embed.setFooter(`${client.config.name}`)
            embed.addFields(
                { name: '👑 Owner / Developer :', value: '<@658657618520440832>', inline: true },
                ...
                { name: '📞 Support :', value: `[Clique ICI Pour Rejoindre Le Support](https://discord.gg/omaji)`, inline: true },
